        ...
        <div class="container-fluid my-2">
            <div class="row mb-2">
                ...
                <div class="col-sm-6 text-right">
                    <a href="{{route('weights.create')}}" class="btn btn-primary">New Weight Range</a>
                </div>
            </div>
        </div>
        <!-- /.container-fluid -->
                ...
                <form action="" method="post">
                    <div class="card-header">
                        <div class="card-title">
                            <button type="button" onclick ="window.location.href='{{route("weights.list")}}'" class="btn btn-default btn-sm">reset</button>
                        </div>
                    ...
                    <table class="table table-hover text-nowrap">
                        ...
                        @if ($weight->isNotEmpty())
                            @foreach ($weight as $weights)
                                <tr>
                                    <td>{{$weights->id}}</td>
                                    <td>{{$weights->min_weight}}</td>
                                    <td>{{$weights->max_weight}}</td>
                                    <td>{{$weights->price}}</td>
                                    {{-- <td>{{$page->content}}</td> --}}
                                    
                                    <td>
                                        <a href="{{route('weights.edit',$weights->id)}}" >
                                            <svg class="filament-link-icon w-4 h-4 mr-1" xmlns="http://www.w3.org/2000/svg" viewBox="0 0 20 20" fill="currentColor" aria-hidden="true">
                                                <path d="M13.586 3.586a2 2 0 112.828 2.828l-.793.793-2.828-2.828.793-.793zM11.379 5.793L3 14.172V17h2.828l8.38-8.379-2.83-2.828z"></path>
                                            </svg>
                                        </a>
                                        <a href="#" onclick="destroyPage({{$weights->id}})" class="text-danger w-4 h-4 mr-1">
...
function destroyPage(id){
    var url = '{{ route("weights.delete", "ID") }}';
